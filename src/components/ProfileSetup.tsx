...
import { User, GraduationCap, Users, LogIn, AlertCircle, Lock, Eye, EyeOff } from 'lucide-react';
import { UserProfile } from '../types';

interface ProfileSetupProps {
  onProfileCreate: (username: string, password: string, profile: UserProfile) => void;
  onSwitchToLogin: () => void;
}

const ProfileSetup: React.FC<ProfileSetupProps> = ({ onProfileCreate, onSwitchToLogin }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Validation
    if (!username.trim() || !password.trim() || !confirmPassword.trim() || !name.trim() || !gradeLevel.trim() || !section.trim()) {
    ...
    if (name.trim() && gradeLevel.trim() && section.trim()) {
      ...
      onProfileCreate(username.trim(), password, profile);
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-white flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md transform transition-all duration-300 hover:scale-105">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">Set up your profile to start learning fractions</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            <label htmlFor="gradeLevel" className="flex items-center space-x-2 text-sm font-semibold text-gray-700">
              <User size={16} />
          ...
          <div className="space-y-2">
            <label htmlFor="section" className="flex items-center space-x-2 text-sm font-semibold text-gray-700">
              <Users size={16} />
              <span>Section</span>
            </label>
            <input
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full bg-gradient-to-r from-blue-500 to-blue-600 text-white py-4 rounded-xl font-semibold text-lg hover:from-blue-600 hover:to-blue-700 transform hover:scale-105 transition-all duration-200 shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
          >
